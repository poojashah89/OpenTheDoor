...
for path, subdirs, files in os.walk('train'):
   for filename in files:
       imagesDir.append(filename)
       f = os.path.join(path, filename)
       paths_to_images.append(f)
image_filenames = filter(lambda x: x.endswith('.jpg'), imagesDir)
...
for path_to_image in paths_to_test_images:
    ...
    # Make sure there's exactly one face in the image
    if len(face_encodings_in_image) != 1:
        #print("Please change image: " + path_to_image + " - it has " + str(len(face_encodings_in_image)))
        print(path_to_image + " Unknown")
        exit()
    # Find match for the face encoding found in this test image
    match = find_match(face_encodings, paths_to_images, face_encodings_in_image[0])
    # Print the path of test image and the corresponding match
    print(path_to_image, os.path.dirname(match))
